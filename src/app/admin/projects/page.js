...
export default function AdminProjects() {
  ...
  if (authenticated === null) {
    return (
      <div className="p-10 text-white bg-gray-900 min-h-screen">Checking...</div>
    );
  }
  ...
  return (
    <main className="p-6 min-h-screen bg-gray-950 text-gray-100">
      ...
      <div className="flex flex-col items-center gap-12">
        ...
        {/* Projects List (Below Form - Full Width Grid) */}
        <div className="w-full max-w-6xl grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {projects.map((p) => (
            <div
              key={p._id}
              className="p-4 border border-gray-800 rounded-2xl shadow-xl bg-gray-900 flex flex-col hover:shadow-2xl transition"
            >
              {p.image && (
                <img
                  src={p.image}
